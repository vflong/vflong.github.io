...
要连接 Pod 命名空间，我们可以将 veth 对的一侧分配给根网络命名空间，另一侧分配给 Pod 的网络命名空间。每对 veth 都想跳线一样工作，将两侧连接起来并允许流量在它们之间流动。此设置可以复制到与计算机上一样多的 Pod。图 4 显示了将 VM 上的每个 Pod 连接到根命名空间的 veth 对。
...
现在，我们希望 Pod 通过根命名空间相互通信，为此，我们使用了网桥。
