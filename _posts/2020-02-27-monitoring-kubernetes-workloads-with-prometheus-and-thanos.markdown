...
您非常高兴，一切都按计划进行，您创建了第一个 Kubernetes 集群（3 个主要的云服务商，Azure、AWS 和 GCP 都右一种简单的方法来配置拖过或非托管 Kubernetes 平台），开发您的第一个容器化应用程序，并将其部署到集群中。那很容易，不是吗？
...
不同的组件在内部相互通信，通常使用 HTTP(s)（REST 或 gRPC），其中一些酱 API 暴露给集群外部（Ingress），这些 API 主要用于：
